...
(defun fast-exec/*to-string-nth-char-and-full-commands* (char-and-full-commands n)
  ...
  (let* ((char-as-str
          (char-to-string (-first-item char-and-full-commands)))
        ...
        (setq previous-words
              (delete-dups
               (fast-exec/nth-words-of-full-commands full-commands
                                                     previous-n)))
        (setq joined-previous-words
              (fast-exec-str/join-strings " | " previous-words))))
...
(defun fast-exec/*to-string-groups-by-nth-char-full-commands* (groups n)
  ...
  (--map
   (fast-exec/*to-string-nth-char-and-full-commands* it n)
   groups))


(defun fast-exec/*to-string-full-commands-as-hints-with-nth-chars* (full-commands n)
...
(defun fast-exec/*insert-full-commands-as-hints-with-nth-chars* (full-commands n)
...
Part of Code got from: https://www.emacswiki.org/emacs/AlignCommands#h5o-2"
  (interactive "r\nsAlign regexp: ")
  (align-regexp start end (concat "\\(\\s-*\\)" regexp) 1 1 nil))


(defun fast-exec/*view-full-commands-as-hints-with-nth-chars-in-new-buffer* (full-commands n)
...
(defun fast-exec/*completing-read-full-command-nth-part* (prompt candidates n)
  ...
  (prog1 (read-char) (kill-buffer fast-exec/hints-buffer-name)))


(defun fast-exec/exec (&optional full-commands typed-chars-num char)
...
(defun fast-exec/*handle-nth-char-of-commands* (commands char n)
  ...
  (let ((suitable-full-commands
    ...
    (pcase (length suitable-full-commands)
      ...
      (1
       (message "Your command found! :) ... Executing")
...
This is mean: Extend `FULL-COMMANDS` and decrease typed char of command: `N`."
  ...
  (let* ((first-command (-first-item full-commands))
         (command-all-initials
          (string-to-list
           (fast-exec/full-command-name-initials first-command)))
         (command-initials (-slice command-all-initials 0 n))
         (enumerated-command-initials
          (fast-exec-functools/enumerate command-initials))
         (suitable-commands
          (--reduce-from
           (fast-exec/full-commands-with-excepted-nth-char
            acc
...
`(fast-exec/register-some-keymaps-funcs
    foo
    bar).`"
  `(--map (fast-exec/register-keymap-func it) (quote ,funcs)))
...
* projectile."
  `(--map
    (fast-exec/*enable-builtin-support-function* it)
    (quote ,pkg-names)))
...
(defun fast-exec/reload-functions-chain ()
  ...
  (setq fast-exec/full-commands
        (-flatten-n 1
                    (-map 'funcall fast-exec/keymap-function-chain))))
